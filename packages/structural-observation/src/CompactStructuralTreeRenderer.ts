...
export function renderCompactStructuralNodeForest<TNode>(
  ...
  const hasPropertyMap =
    options.properties !== undefined && options.properties.size > 0;
  ...
  const subtreeHasProperties = (entry: TNode): boolean => {
    ...
    const hasProperties = (options.properties?.get(node.ref)?.length ?? 0) > 0;
    const result =
      hasProperties ||
      source
        .children(entry)
...
function renderStructuralNode<TNode>(
  ...
  const propertyLines = extraPropertyLines(node, depth, options, renderState);
...
function formatNodeHeader(
  ...
  segments.push(`[${identityToken(node, options)}]`, node.role);
...
function extraPropertyLines(
  ...
  const lines = (options.properties?.get(node.ref) ?? []).map(
    (property) =>
...
function formatPropertyValue(
  value: string | readonly string[],
  preservePomPath = false
): string {
  if (preservePomPath && typeof value === "string") return value;
