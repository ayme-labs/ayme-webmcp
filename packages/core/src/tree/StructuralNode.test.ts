...
import { AriaRefSchema } from "./StructuralTypes";
...
const testEnrichment = defineStructuralEnrichment({
  key: "test",
  schema: z.string(),
  properties: { value: (value: string) => value },
...
describe("StructuralNode", () => {
  describe("copy", () => {
    it("overrides selected data while preserving omitted fields and typed enrichments", () => {
      const node = new StructuralNode({
        ref: AriaRefSchema.parse("e1"),
        status: { kind: "updated", selfChanged: true, childListChanged: false },
        childListOrderRelevant: true,
        role: "button",
        name: "Save",
        state: { disabled: true, level: 2 },
        cursorPointer: true,
        props: { title: "Save document" },
        children: ["before"],
      }).attachEnrichment(testEnrichment, "trace");

      const copied = node.copy({ children: ["after"] });

      expect(copied.ref).toBe(node.ref);
      expect(copied.status).toEqual(node.status);
      expect(copied.childListOrderRelevant).toBe(true);
      expect(copied.role).toBe(node.role);
      expect(copied.name).toBe(node.name);
      expect(copied.state).toEqual(node.state);
      expect(copied.cursorPointer).toBe(node.cursorPointer);
      expect(copied.props).toEqual(node.props);
      expect(copied.children).toEqual(["after"]);
      expect(copied.enrichmentValue(testEnrichment)).toBe("trace");
    });

    it("preserves omitted status and clears it when explicitly set to undefined", () => {
      const node = new StructuralNode({
        ref: AriaRefSchema.parse("e1"),
      ...
      expect(node.copy({}).status).toEqual({ kind: "unchanged" });
      expect(node.copy({ status: undefined }).status).toBeUndefined();
    });
  });
...
`.trim()
    ...
    it("deep equality distinguishes structural property values", () => {
      const before = expectNode(
        parse('- link "Docs" [ref=e1]:\n  - /url: https://example.test/docs')
          .root
      );
      const after = expectNode(
        parse('- link "Docs" [ref=e2]:\n  - /url: https://example.test/other')
          .root
      );

      expect(before.isShallowEqual(after)).toBe(true);
      expect(before.isDeepEqual(after)).toBe(false);
      expect(before.isSelfEqualExceptRef(after)).toBe(false);
    });
  });

  describe("descendant changes", () => {
    it("returns false when every structural descendant is unchanged", () => {
    ...
    it("finds a changed descendant at any depth", () => {
      const [before, after] = parsePair(
        '- generic [ref=e1]:\n  - generic [ref=e2]:\n    - button "Save" [ref=e3]',
